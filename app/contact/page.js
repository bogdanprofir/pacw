...
export default function ContactPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Basic validation: all fields are required, and at least one interest must be selected, GDPR must be checked.
    if (
      !formData.name ||
      !formData.surname ||
      !formData.email ||
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="text-center mb-8">
        <Image src="/logo.png" alt="PAC Water Logo" width={150} height={100} className="mx-auto" />
      ...
      {interestOptions.map((interest, index) => (
        ...
        <div className="flex items-center">
          ...
          <span className="text-sm">
            Sunt de acord cu <Link href="/gdpr" className="underline text-blue-600">Politica de Confidențialitate GDPR</Link>.
          </span>
        </div>
        <button
          type="submit"
